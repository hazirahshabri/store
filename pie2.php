...
$sql1 = "select count(totalBag) AS QUANTITY_BAG, count(otherItem) AS QUANTITY_OTHERITEM from item";
        ...
        margin-left:950px;
           ...
           {  
                var data = google.visualization.arrayToDataTable([  
                          ['QUANTITY_BAG', 'QUANTITY_OTHERITEM'],  
                          <?php  
                          while($row = mysqli_fetch_array($run1))  
                          {  
                               echo "['".$row["QUANTITY_BAG"]."', ".$row["QUANTITY_OTHERITEM"]."],";  
                          }  
                          ?>  
                     ]);  
                var options = {  
